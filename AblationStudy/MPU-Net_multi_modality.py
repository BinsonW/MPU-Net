    ...
    def forward(self, skips):
        ...
        for i in range(len(self.tus)):
            ...
            if self.deep_supervision:
                skips_out.append(self.deep_supervision_outputs[i](out))
...
class MPUNet_multi_modality(nn.Module):
    def __init__(
        self,
        ...
        super(MPUNet_multi_modality, self).__init__()
        self.num_classes = num_classes
    ...
    def forward(self, x):
        skips_enc_plain = self.encoder_plain(x)
        out = self.decoder_plain(skips_enc_plain)
        return out
