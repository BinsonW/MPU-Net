        ...
        for i, s in enumerate(np.arange(num_stages)[::-1]):
            ...
            if deep_supervision:
                seg_layer1 = self.props["conv_op"](
                    features_skip, features_skip, 1, 1, 0, 1, 1,
...
class MPUNet_single_modality(nn.Module):
    def __init__(
        self,
        ...
        super(MPUNet_single_modality, self).__init__()
        self.num_classes = num_classes
    ...
    def forward(self, x):
        ...
        out = self.decoder_groupconv(skips_enc_dw)
        return out
